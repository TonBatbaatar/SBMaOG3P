...
using System;
using System.Linq;
using HeuristicLab.Algorithms.CFG.MultiObjective;
...
namespace HeuristicLab.Algorithms.GeneticAlgorithm
{
    ...
    /// A genetic algorithm.
    /// </summary>
    [Item("Genetic Algorithm (GA) keep inserted Solution at each generation", "A genetic algorithm keep inserted Solution at each generation.")]
    [Creatable(CreatableAttribute.Categories.Algorithms, Priority = 100)]
    [StorableClass]
    public sealed class GeneticAlgorithmKeepInsertedSolution : HeuristicOptimizationEngineAlgorithm, IStorableContent
    {
        ...
        private ValueParameter<ILog> LogParameter
        {
        ...
        private GeneticAlgorithmKeepInsertedSolutionMainLoop GeneticAlgorithmMainLoop
        {
        ...
        public GeneticAlgorithmKeepInsertedSolution()
          : base()
        {
            ...
            GeneticAlgorithmKeepInsertedSolutionMainLoop mainLoop = new GeneticAlgorithmKeepInsertedSolutionMainLoop();
            ...
            solutionsCreator.InitialGivenProgramCreator.Value.LogParameter.ActualName = LogParameter.Name;
            ...
            ISelector proportionalSelector = SelectorParameter.ValidValues.FirstOrDefault(x => x.GetType().Name.Equals("TournamentSelectorWithSolutionInsertion"));
        ...
        [StorableConstructor]
        private GeneticAlgorithmKeepInsertedSolution(bool deserializing) : base(deserializing) { }
        ...
        private GeneticAlgorithmKeepInsertedSolution(GeneticAlgorithmKeepInsertedSolution original, Cloner cloner)
        ...
        {
            return new GeneticAlgorithmKeepInsertedSolution(this, cloner);
        }
        ...
        private void ParameterizeSelectors()
        {
            foreach (ISelector selector in SelectorParameter.ValidValues)
            {
                ...
                selector.NumberOfSelectedSubScopesParameter.Value = new IntValue(2 * (PopulationSizeParameter.Value.Value - ElitesParameter.Value.Value - 1));
        ...
        private GeneticAlgorithmKeepInsertedSolutionMainLoop FindMainLoop(IOperator start)
        {
            IOperator mainLoop = start;
            while (mainLoop != null && !(mainLoop is GeneticAlgorithmKeepInsertedSolutionMainLoop))
                mainLoop = ((SingleSuccessorOperator)mainLoop).Successor;
            if (mainLoop == null) return null;
            else return (GeneticAlgorithmKeepInsertedSolutionMainLoop)mainLoop;
        }
        #endregion
    }
}
